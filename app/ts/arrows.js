"use strict";
const canvasOverlay = document.querySelector("#arrow_canvas");
...
class Arrow {
     ...
     */
    constructor(startPos, endPos, pathDirrect, style, transitionBezierCurve = Arrow.DefaultTransitionBezierCurve) {
        this.uuid = uuid4();
     ...
     */
    draw(ctx) {
        const Logs = Arrow.DrawLogs.get(this.uuid);
        ...
        //@ts-expect-error
        const bezierFactor = bezierInterpolation(this.transitionBezierCurve, transitionFactor);
        ...
        if (vecFromStartToEnd.length() < 0.3)
            return;
         ...
         * @param v vector
         */
        const CTRCTA = (v) => ChessToRealCoordinates(v, units).toArray();
        const makeArrowColorGradient = (centerStartPoint, unitDirVector, centerEndPoint) => {
     ...
     */
    static renderArrows(ctx, logs = false) {
 ...
 */
Arrow.DrawLogs = new Map();
/**
 * all actively drawn arrows
 */
Arrow.ActiveArrows = new Set();
Arrow.DefaultSyles = {
    move: {
        outline: "transparent",
...
Arrow.DefaultTransitionBezierCurve = {
    points: {
        p1: new Vector(0, 0),
        p2: new Vector(1, 0),
        p3: new Vector(1, 1),
 ...
 */
function ChessToRealCoordinates(chessCoord, unit) {
 ...
 */
function visuallizeBezierCurve(p1, p2, p3, p4, period, resolution) {
    try {
        const units = getChessToRealCoordUnits();
        ...
        const interp = (t) => bezierInterpolation(P1, P2, P3, P4, t, "point");
        units.y = units.x;
